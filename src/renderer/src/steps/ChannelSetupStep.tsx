...
import type { SetupPayload } from '../constants/setup'
...
export default function ChannelSetupStep({
  ...
  const channelTitle = channelType
    ? t(`channelGuide.channels.${channelType}.title`)
    : t('setup.noChannelTitle')
  const hasExecutionOutput = running || !!status || logs.length > 0 || !!error
  const modeLabel = skippedChannelConfig
  ...
  const nextHint = skippedChannelConfig
                ...
                <div className="grid gap-3 md:grid-cols-3">
                  {infoCards.map((card) => (
                    ...
                    >
                      ...
                      <p className="mt-2 text-sm font-bold leading-6 text-text">{card.value}</p>
                    </div>
                  ))}
                </div>
